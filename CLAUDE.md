...
- [x] **Fix: capture the on-screen preview itself.** The right-panel preview map is already mounted, loaded, fitted, and showing markers/route/labels — it *works*. `handleExport` now calls `domToPng(previewRef.current, { scale: format.width / previewWidth, width: previewWidth, height: previewHeight, backgroundColor })` — it captures that element and **upscales** to the target format. Text/pills/leader lines re-rasterize crisply from computed styles; the MapLibre canvas is read via `toDataURL` (`preserveDrawingBuffer:true`, already set). **WYSIWYG — the export now always matches the preview**, and there's no second map, no cold-load race, no off-screen/iOS compositing issue. Removed: the hidden capture `ExportStage`, the `left:-9999`/scaled-wrapper + covering overlay, `captureRef`/`readyResolveRef`/`handleCaptureReady`/`onReady`-await, `EXPORT_READY_TIMEOUT_MS`.
- [x] **Quality note:** map-tile sharpness = preview canvas buffer = `previewWidth × devicePixelRatio`. On a phone (DPR ~3) that's ~1000–1100 px, so the 1080-wide formats come out ~1:1; the 1920 format is a mild upscale; desktop (DPR 1) is softer (but export is a phone workflow). If crisper is ever needed, temporarily raise the preview map's `pixelRatio` before capture (don't recreate the map). `backgroundColor` fills behind the preview's rounded corners so they don't export as transparent notches.
- Kept from round 4 (still valid): `MapView`/`MapWrapper` `pixelRatio` prop (MapLibre v4 constructor option, general-purpose) and the `offsetWidth`/`offsetHeight` label measurement (transform-agnostic). **Still not device-verified from the dev env (no iOS/browser here).**
