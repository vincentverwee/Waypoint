...
- [x] Canvas/html2canvas export — captures a dedicated full-resolution off-screen map instance (not the on-screen preview) via `html2canvas`, so the exported PNG is crisp regardless of viewport size. Requires `preserveDrawingBuffer: true` on the MapLibre instance (`MapView.tsx`) — without it, WebGL discards its buffer and html2canvas can capture a blank frame.
