...
### Debugging Milestone 5 (round 6) — the *download* never fired on iPhone
- [x] **Symptom (real device, iOS Safari on the Vercel deploy):** tap “Export PNG” → the screen flashes → nothing. No file, no error. Rounds 1–5 all chased the *rendering*; the render was fine — the **save** was what failed. Three stacked causes:
  1. **The download outlived its user gesture.** `handleExport` awaited `document.fonts.ready`, a 350 ms settle, and then `domToPng` (seconds on a 1080×1920 node). iOS Safari only honours a programmatic download/share while the tap's *transient user activation* is alive — long gone by then, so `link.click()` was silently ignored.
  2. **Multi-megabyte `data:` URL.** A 1080×1920 PNG base64-encoded is several MB; iOS Safari refuses to download a `data:` URL that large (the brief "flash" = Safari starting to handle the URL and bailing).
  3. **No `catch` at all** — `handleExport` had `try`/`finally` only, so any throw became an unhandled rejection while `finally` reset the button. Every failure mode looked identical: "nothing happened".
- [x] **Fix — split render and save into two taps.** `handleRender()` rasterizes to a **Blob** (`domToBlob`, not `domToPng` — object URLs have no size limit) and parks it in state with an object URL; the sidebar then shows the finished image plus a **Save** button. `handleSave()` runs inside that *second, fresh* tap, so the activation is valid. On iOS it calls `navigator.share({ files: [file] })` — the share sheet's "Save Image" is the only real route into Photos — with **no `await` before the `share()` call** (an await drops the activation); everywhere else it does a normal object-URL anchor download (anchor appended to the document, which Firefox requires). The rendered image is also displayed inline, so long-press → "Add to Photos" always works as a last resort.
- [x] **Failures are now visible** — `handleRender` catches and renders the message in the sidebar, so a genuine capture failure can no longer masquerade as a dead button.
